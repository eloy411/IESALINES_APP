  ...
  actions: {
    async getCategoriasPorcentaje() {
      try {
        ...
        const res = await api.get(
          "http://localhost:8000/api/admin/ronda/subcat-porcentajes"
        );
    ...
    async deleteSubCategoriesVotaciones(rowToDelete) {
      try {
        ...
        const res = await api.delete(
          `http://localhost:8000/api/admin/ronda/subcat-votaciones/${rowToDelete.id_subcategoria}`
        );
        console.log(res);
    ...
    async getSubCategoriasIndependent() {
      try {
        ...
        const res = await api.get(
          "http://localhost:8000/api/admin/subcategorias"
        );
        console.log(res);
    ...
    async getSubCategoriasAuxFromSelect(tipoJurado) {
      try {
        ...
        const res = await api.get(
          `http://localhost:8000/api/admin/aux-subcategorias/${tipoJurado}`
        );
        console.log(res);
        console.log(this.subCategoriasArrAux);
            ...
            for (let i = 0; i < res.data.length; i++) {
              for (let j = 0; j < this.subCategoriasArr.length; j++) {
                if (res.data[i] == this.subCategoriasArr[j].id) {
                  this.subcategoriasMutationJuradoEnabled.push(
                    this.subCategoriasArr[j]
                  );
                  this.subCategoriasArr = this.subCategoriasArr.filter(
                    (subcat) => subcat.id != res.data[i]
                  );
                }
              }
            }
    ...
    async putSubCategoriasAux(tipoJurado, idEdicion) {
      try {
        ...
        let idArr = [];
        this.subcategoriasMutationJuradoEnabled.forEach((subcat) =>
          idArr.push(subcat.id)
        );
        console.log(idArr);
        const res = await api.put(
          `http://localhost:8000/api/admin/aux-subcategorias`,
          {
    ...
    async getSubCategoriasFromResult() {
      try {
        ...
        const res = await api.get(
          `http://localhost:8000/api/admin/ronda/subcat-result`
        );
        console.log(res);
    ...
    async putOtorgarPremio() {
      try {
        ...
        const res = await api.put(
          `http://localhost:8000/api/admin/ronda/premio`,
          {
    ...
    async getVotosJuradoResult() {
      try {
        ...
        const res = await api.get(
          `http://localhost:8000/api/admin/ronda/jurados-result/${this.id_subcategoria}`,
          {
